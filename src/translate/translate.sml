structure translate :> translate = struct
  ...
  val outermost: level = {parent=NONE, frame=newFrame{name="_main"}, level=getActualLev()}
  ...
  (* cambiamos esto por la linea de abajo para que aguarde todas las variable en
     el stack fun allocLocal{parent, frame, level} b = frame.allocLocal frame true *)
  ...
  local
    val salidas: label option stack.Pila = (stack.nuevaPila1 NONE)
  in
    val pushSalida = stack.pushPila salidas
    fun popSalida() = stack.popPila salidas
    fun topSalida() =
        case stack.topPila salidas of
          SOME l => l
  ...
  fun addAccFrame access (level:level) =  frame.addAccFrame access (#frame level) (*((#frame) level) @ [access]*)
