structure assem = struct
	...
	val format =
		...
		in	fn OPER{assem,dst,src,jump=NONE} =>
				"\t"^speak(assem,
					List.map temp.temp2string dst,
					List.map temp.temp2string src,nil)
			| OPER{assem,dst,src,jump=SOME j} =>
				"\t"^speak(assem,
					List.map temp.temp2string dst,
					List.map temp.temp2string src,j)
			| LABEL{assem,...} => assem
			| MOVE{assem,dst,src} =>
				"\t"^speak(assem,
					[temp.temp2string dst],
					[temp.temp2string src],nil)
		end
