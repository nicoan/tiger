...
struct
  ...
  fun coloring (b, f, firstRun) =
      let
        ...
        val _ = insertNodesLiv (IGRAPH ig) (aux.unionList String.compare totalRegs precoloredList)
        ...
        val color:((temp.temp, string) table.Tabla) ref = ref (table.fromList [("eax", "eax"), ("ebx", "ebx"),("ecx", "ecx"),("edx", "edx"),("esi", "esi"),("edi", "edi")])
        ...
        val selectStack: node stack ref = ref aux.emptyStack
        val adjSet =
          let
        ...
        fun selectSpill () =
            let
              fun spillPriority (n) =
                  let
                    val nDef = List.foldr (fn ((x, y), xs) => if aux.inList (nodeToTemp (IGRAPH ig) n)  y then 1.0 + xs else xs) 0.0 (table.tabAList (!(#def fg)))
                    val nUse = List.foldr (fn ((x, y), xs) => if aux.inList (nodeToTemp (IGRAPH ig) n) y then 1.0 + xs else xs) 0.0 (table.tabAList (!(#use fg)))
                    val nAdj = Splayset.numItems(sub(!adjList, n))
                  in
                    if aux.inList (nodeToTemp (IGRAPH ig) n) ["eax", "ebx", "ecx", "edx", "esi", "edi"] then 9999999.0 else (nDef + nUse) / real(nAdj)
                  end
        ...
        fun assignColors () =
            let
              ...
              fun processAdjList n = Splayset.app (fn w =>
                      let
        ...
        fun rewriteProgram () =
            let
              ...
              val memLocsTab = table.fromList ((List.map (fn s => (s, getNewAlloc()))) (Splayset.listItems (!spilledNodesTmp)))


              fun makeFetch oldT newT=
                  let
                    val m = case table.tabBusca(oldT, memLocsTab) of
                        SOME mem => mem
                      | NONE => raise Fail("No hay memoria alocada para el temporal " ^ oldT ^ "\n")
                  in
                    OPER {assem="movl " ^ m ^ "(%ebp), `d0" , dst=[newT], src=[], jump=NONE}
                  end
              ...
              fun makeStore oldT newT=
                  let
                    val m = case table.tabBusca(oldT, memLocsTab) of
                      ...
                      | NONE => raise Fail("No hay memoria alocada para el temporal " ^ oldT ^ "\n")
                  in
                    OPER {assem="movl `s0, "^ m ^ "(%ebp)", dst=[], src=[newT], jump=NONE}
                  end

              fun rewriteInstruction (i as LABEL l) = [i]
                | rewriteInstruction i =
                  let
                    val spillUses = Splayset.listItems(Splayset.intersection(!spilledNodesTmp, Splayset.addList (Splayset.empty String.compare, assem.src2List i)))
                    val spillDefs = Splayset.listItems(Splayset.intersection(!spilledNodesTmp, Splayset.addList (Splayset.empty String.compare, assem.dst2List i)))
                    val newtempsTab = table.fromList (List.map (fn t => let
                              val t' = temp.newtemp()
                              val _ = newTempsC := Splayset.add(!newTempsC, t')
                            in (t, t') end) (aux.unionList (String.compare) spillUses spillDefs))
                    fun newTemps x = case table.tabBusca (x, newtempsTab) of
                          SOME t => t
                        | NONE => x
  ...
  fun replaceTforColors instrs colors =
      let
        fun getColor t =
            let
              val c = case table.tabBusca(t, colors) of
                  SOME col => col
                | NONE => t
            in
              c
