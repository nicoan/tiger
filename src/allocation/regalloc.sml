...
struct
  ...
  fun simpleregalloc (frm:frame.frame) (body:instr list) =
      let
        ...
        val precolored = ["eax", "ebx", "ecx", "edx", "esi", "edi", "ebp", "esp"]
        ...
        val asignables = ["eax", "ebx", "ecx", "edx", "esi", "edi"]
        ...
        fun movaMem(temp, mempos) =
            ...
            in
              OPER {assem="movl `s0 (%ebp" ^ desp ^ ")", src=[temp], dst=[], jump=NONE}
            end
        fun movaTemp(mempos, temp) =
            let
              val desp = if mempos<0 then " - " ^ Int.toString(~mempos) else if mempos>0 then " + " ^ Int.toString(mempos) else ""
            in
              OPER {assem="movl (%ebp" ^ desp ^ ") `d0", src=[], dst=[temp], jump=NONE}
            end
        val temps =
          let
            val tempList =
